...
class _HeaderControlState extends State<HeaderControl> {
  ...
  void showPlayerInfo() {
    ...
    if (player == null) {
      SmartDialog.showToast('Player Not Initialized');
      return;
    }
    showDialog(
      context: context,
      ...
      child: Column(
        children: [
          ...
          ListTile(
            ...
            title: const Text('Video Quality', style: titleStyle),
            subtitle: Text(
              '${widget.videoDetailCtr!.currentVideoQa.description}',
              style: subTitleStyle,
            ),
          ),
          if (widget.videoDetailCtr!.currentAudioQa != null)
            ListTile(
              ...
              title: const Text('Audio Quality', style: titleStyle),
              subtitle: Text(
                '${widget.videoDetailCtr!.currentAudioQa!.description}',
                style: subTitleStyle,
              ),
            ),
          ListTile(
            ...
            subtitle: Text(
              '${widget.videoDetailCtr!.currentDecodeFormats.description}',
              style: subTitleStyle,
            ),
          ),
          ListTile(
            ...
            title: const Text('Playback Type', style: titleStyle),
            subtitle: Text(
          ...
          ListTile(
            ...
            title: const Text('Danmaku Settings', style: titleStyle),
          ),
          ListTile(
            title: const Text('Playback Info', style: titleStyle),
          ...
          ListTile(
            title: const Text('CDN Server', style: titleStyle),
            leading: Icon(MdiIcons.cloudPlusOutline, size: 20),
            subtitle: Text(
              'Set to: ${CDNServiceCode.fromCode(defaultCDNService)!.description} (Advanced Users Only)',
              style: subTitleStyle,
            ),
            onTap: () async {
              Get.back();
              ...
              if (result != null) {
                ...
                SmartDialog.showToast(
                  'Set to ${CDNServiceCode.fromCode(result)!.description}，Reloading Video',
                );
                setState(() {});
                widget.videoDetailCtr!.queryVideoUrl();
              }
                ...
                return TextButton(
                  onPressed: isSending
                      ...
                      : () async {
                          ...
                          if (msg.isEmpty) {
                            SmartDialog.showToast('Comment cannot be empty');
                            return;
                          ...
                          if (res['status']) {
                            ...
                            SmartDialog.showToast(
                              'Sending failed, error message is ${res['msg']}',
                            );
                          }
                        },
                  child: Text(isSending ? 'Sending...' : 'Sent'),
                );
              },
            ),
          ],
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    const Center(
                      child: Text('Timed Shutdown', style: titleStyle),
                    ),
                    ...
                    for (final int choice in scheduleTimeChoices) ...<Widget>[
                      ListTile(
                        ...
                        dense: true,
                        title: Text(
                          choice == -1 ? "Disable" : "$choice minutes later",
                        ),
                        trailing:
                    ...
                    Row(
                      children: <Widget>[
                        const Text('Trigger Timing：', style: titleStyle),
                        const Spacer(),
                        ActionRowLineItem(
                          onTap: () {
                    ...
                    Row(
                      children: <Widget>[
                        const Text('Trigger Action：', style: titleStyle),
                        const Spacer(),
                        ActionRowLineItem(
                          onTap: () {
  ...
  void showSetVideoQa() {
    if (videoInfo.dash == null) {
      SmartDialog.showToast(
        'This video does not support selecting video quality',
      );
      return;
    }
          ...
          children: <Widget>[
            SizedBox(
              ...
              child: GestureDetector(
                onTap: () {
                  SmartDialog.showToast(
                    'High video quality requires a Bilibili membership (already a member? Please turn off the incognito mode); 4K and Dolby Vision playback may not be good',
                  );
                },
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    const Text('Select Video Quality', style: titleStyle),
                      ...
                      for (int i = 0; i < totalQaSam; i++) ...[
                        ListTile(
                          onTap: () {
                            ...
                            Get.back();
                            SmartDialog.showToast(
                              "Default: $oldQualityDesc Selected: ${VideoQualityCode.fromCode(quality)!.description}",
                            );
      ...
      Container(
        ...
        child: Column(
          children: <Widget>[
            const SizedBox(
              height: 45,
              child: Center(child: Text('Audio Quality', style: titleStyle)),
            ),
            Expanded(
              child: SingleChildScrollView(
                child: Column(
                    ...
                    for (final AudioItem i in audio) ...<Widget>[
                      ListTile(
                        onTap: () {
                          ...
                          Get.back();
                          SmartDialog.showToast(
                            "Default: $oldQualityDesc Selected: ${AudioQualityCode.fromCode(quality)!.description}",
                          );
                          widget.videoDetailCtr!.updatePlayer();
                        },
                        dense: true,
  ...
  void showSetDecodeFormats() {
    ...
    if (list == null) {
      SmartDialog.showToast(
        'This video does not support the selected decoding format',
      );
      return;
    }
      ...
      Container(
        ...
        child: Column(
          children: [
            const SizedBox(
              height: 45,
              child: Center(child: Text('Decoding Format', style: titleStyle)),
            ),
            Expanded(
              child: SingleChildScrollView(
                child: Column(
  ...
  void showSetDanmaku() async {
    ...
    final List<Map<String, dynamic>> blockTypesList = [
      {'value': 5, 'label': 'Top', 'icon': Icons.vertical_align_top},
      {'value': 6, 'label': 'Bottom', 'icon': Icons.vertical_align_bottom},
      {'value': 7, 'label': 'Color', 'icon': Icons.palette},
      {'value': 8, 'label': 'Advanced', 'icon': MdiIcons.paletteAdvanced},
    ];
                ...
                child: Column(
                  ...
                  children: [
                    const SizedBox(
                      height: 45,
                      child: Center(
                        child: Text('Danmaku Settings', style: titleStyle),
                      ),
                    ),
                    const SizedBox(height: 10),
                    Row(
